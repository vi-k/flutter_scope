...
Ветка `main` (`bea96c2`), рабочее дерево чистое, других веток и worktree нет.
...
Прогон 2026-08-13: `fvm flutter test` — **143 теста, все зелёные**;
`dart format` без изменений; `dart doc` 0/0; `pub publish --dry-run` 0
предупреждений; `analyze` чист в обоих примерах, в корне — 2 замечания
(см. ниже).
...
2. **Публикация 0.10.0** — тесты, `dart doc` и `pub publish --dry-run` чистые;
   до публикации стоит закрыть 2 замечания `analyze`, решение о выпуске — за
   владельцем.
