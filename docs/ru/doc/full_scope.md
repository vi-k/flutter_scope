...
> Перевод `doc/full_scope.md` (blob `87c7a17bb515bd6e486a4e658f59e97d3813133c`).
...
### Провалившаяся инициализация разбирается изнутри

Шесть шагов выше — обычный путь, тот, на котором скоуп стал готовым.
Провалившаяся инициализация идёт другим маршрутом, и разницу стоит прочитать
дважды: половина последовательности просто не происходит.

| что выполняется | готов, потом ушёл | инициализация провалилась |
| --- | --- | --- |
| `ScopeState.onUnmount` | да | **состояния нет** |
| `dep.unmount` | да, у каждой зависимости | да, у каждой зависимости |
| `ScopeState.disposeAsync` | да | **состояния нет** |
| `dep.dispose` | да, в обратном порядке | да, в обратном порядке |

**Состояние — это та часть, которой не было вовсе.** `createState()`
выполняется, когда строится готовая ветка, а провалившийся скоуп её не строит —
значит, `ScopeState.onUnmount` и `ScopeState.disposeAsync` не на чем выполнять.
Это то же правило, что тема `AsyncScope` формулирует для тамошнего `dispose`, и
держится оно по той же причине: эти хуки написаны про законченный скоуп, а
насколько далеко зашла постройка, знает только тот код, который её вёл.

**Зависимости — это та часть, которая была**, и их возвращают — только не
элемент. Элементу контейнер вручают вместе с `ScopeReady`, так что на этом пути
у него его никогда и не было. Вместо элемента контейнер разбирает себя сам,
изнутри собственной инициализации, — это и есть
...
написанный вами про законченный скоуп, на незаконченном не выполняется; вещь,
...
Отсюда правило большого пальца, то же, что в теме `AsyncScope`. Что
инициализация взяла до провала, инициализация и возвращает — через
`dep.dispose`, если брали через `dep`, и своими руками, если нет:
