...
> Перевод `doc/full_scope.md` (blob `b42c81773f7920de22efeff8c11745ea7aa81ada`).
...
2. **Состояние.** `createState()`, потом `initState()` — где `dependencies`
   уже на месте, ради чего семейство и заводили, — потом `initAsync()`,
...
| что выполняется | готов, потом ушёл | провалился `initDependencies` | провалился `initAsync` состояния |
| --- | --- | --- | --- |
| `ScopeState.initState` | да | **состояния нет** | да |
| `ScopeState.onUnmount` | да | — | **да** |
| `dep.unmount` | да, у каждой | да, у каждой | да, у каждой |
| `ScopeState.disposeAsync` | да | — | **нет** |
...
`ScopeState.disposeAsync`, потом `dep.dispose`. Группу обе половины обходят
...
`ScopeState.disposeAsync` выполнять не на чем. Не кладите в них освобождение
того, что взято во время `initDependencies`: на том пути, где это важнее всего,
их там нет.

**Когда провалился `initAsync` состояния, состояние есть, его отмонтируют, но не
утилизируют.** `onUnmount()` выполняется — это синхронная половина, а состояние,
дошедшее до `initState()`, уже может держать подписку. `disposeAsync()` — нет:
...
`ScopeState.disposeAsync` здесь и `dispose` в темах `AsyncScope` и
...
То же и для собственной половины состояния: `initAsync()`, который что-то взял
и потом бросил, обязан освободить это до броска — `disposeAsync()` его для
этого не позовут.
