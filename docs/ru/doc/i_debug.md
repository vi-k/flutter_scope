...
> Перевод `doc/i_debug.md` (blob `e328f9015f5dfb2c183797f47472c41627c51061`).
...
Два ожидания в жизненном цикле скоупа ограничены таймаутом, и оба значения по
умолчанию живут в `ScopeConfig`:
...
Оба по умолчанию три секунды. `null` снимает ограничение, и скоуп ждёт
...
Любой скоуп может переопределить оба значения для себя параметрами
`scopeKeyTimeout` и `waitForChildrenTimeout` и заметить истечение через колбэки
`onScopeKeyTimeout` и `onWaitForChildrenTimeout`.
