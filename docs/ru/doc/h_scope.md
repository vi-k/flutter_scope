...
> Перевод `doc/h_scope.md` (blob `89fa0d94562666354cda2d275d03c7ffba3f7681`).
...
- `dep(name, init)` — одна зависимость. `DepHelper`, который передают в `init`, —
  это место, где регистрируют обратные операции: `dep.dispose` дожидаются при
  утилизации, `dep.unmount` зовут синхронно, когда скоуп уходит с дерева. Не
  задать ни того ни другого нормально — зависимости, которая ничем не владеет,
  разбирать нечего. Имя не может быть пустым.
...
1. `unmount` — синхронный, зовётся в тот момент, когда скоуп убирают с дерева
   виджетов, и до начала любого асинхронного шага. Скоуп пробрасывает его в
   `ScopeDependencies.unmount`, а `ScopeAutoDependencies` — дальше в каждый
   `dep.unmount`. Здесь место всему, что должно случиться немедленно и не может
   ждать асинхронного разбора: отписаться, например.
