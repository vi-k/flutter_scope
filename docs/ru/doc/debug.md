...
> Перевод `doc/debug.md` (blob `d447a3b608c078aa9fffb7e59f90528886764f7c`).
...
защищена сама по себе, так что упасть могут обе: первый отказ уходит ещё и
броском, второй — только отчётом, но `onError` несёт оба.
...
шесть, — и `what` называет то, которое истекло:
...
Пятое ожидание берёт одно из этих умолчаний и своего переопределения не имеет.
