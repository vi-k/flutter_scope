...
> Перевод `doc/debug.md` (blob `0867e7b453186c9ecd3d8d179736f560d9b7db41`).
...
когда позже добавят десятый; делегат — единственный наследник, которому от них
...
Четыре в середине — это хуки контейнера зависимостей, и они складываются в две
пары: вход и выход на каждую половину его жизненного цикла. Оба входа шлются
изнутри шага и до того, как он чего-либо дождётся, — этим пары и ценны, см.
«Последний вход без выхода» ниже.
...
- **зависимость вашего собственного изготовления входа не объявляет.** Той,
  ...
  `sequential` или `concurrent`, отметку взять неоткуда. Её `onProgress`
  по-прежнему приходит: эта половина едет потоком `init()`, то есть ровно той
  частью договора, которую такая зависимость и реализует.
...
`FlutterError.reportError`, если только его место не занял ваш колбэк.
Переданный вами `onTimeout` в `waitForChildren` заменяет этот отчёт, а не это
событие: наблюдатель — то, что пакет сообщает о себе, а не ваша обработка
ошибок.
...
Ничего не перестаёт компилироваться, и в этом неудобство именно этой правки:
наблюдатель, чей `onProgress` разбирал обе половины, по-прежнему переопределяет
существующий метод — просто для разбора его больше не зовут. Если вы различали
