...
Об истечении через наблюдателя сообщают три ограниченных ожидания, и `what`
называет то, которое истекло: `its own teardown` (скоуп ждёт `disposeScope`),
`its initialization to be cancelled` и `the disposal` (контейнер зависимостей
ждёт освобождения построенного им дерева).
...
ушёл бы в бесконечную рекурсию; второе оповещение отбивается и репортится один
раз, а первое доходит до конца.
