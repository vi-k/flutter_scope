...
**Вложенная навигация уехала.** `NavigationNode` жил здесь до 0.10.0
включительно, а теперь это пакет
[navigation_node](https://pub.dev/packages/navigation_node) — вложенный
`Navigator`, который удерживает диалоги, шторки и открытые экраны под тем
скоупом, из которого их открыли. Он не зависит ни от чего, кроме Flutter, —
потому и уехал; в паре они работают ровно как работали. Добавьте пакет и
поправьте один импорт.
