...
> **Состояние на 2026-08-19:** выполнено целиком, ветка `observer` (в `main`
> не влито):
> `cc59355` (каркас и хуки), `348dbc6` (скоуп), `0e1cada` (зависимости и
> потоки), `a852b93` (печать, удаление журнала, `logger_builder` из
> репозитория), `5f1987e` (названия фаз по-английски), `7d631ee`
> (документация), `1fa18aa` и `d23b43c` (правки по её ревью). Дизайн выполнен
> целиком; разошлось одно место. §4 обещала, что базовую пару
> `onInit`/`onDisposed` можно поставить в `ScopeWidgetElementBase` так, чтобы
> событие не задваивалось: это было написано без проверки, что
> `LiteScopeElementBase extends AsyncScopeElementBase`, — то есть асинхронные
> семейства наследуют базовую пару и никакая точка постановки от задвоения
> не спасает. Развёл их переключатель
> `ScopeWidgetElementBase.reportsOwnLifecycle` (`false` в базе,
