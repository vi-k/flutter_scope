...
> (документация) и `1fa18aa` (правки по её ревью). Дизайн выполнен целиком;
> разошлось одно место. §4 обещала, что базовую пару `onInit`/`onDisposed`
> можно поставить в `ScopeWidgetElementBase` так, чтобы событие не
> задваивалось: это было написано без проверки, что
