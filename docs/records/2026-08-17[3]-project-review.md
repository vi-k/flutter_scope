...
**Вердикт: исправлено.** Волна 15, блок инфраструктуры, `b4aa330` и `c625776`.
Заведён `.github/workflows/ci.yml`: восемь команд §6 в том же порядке, на
объявленном полу (Flutter 3.29.0 ставит действие, `fvm` там не нужен), после них
— два сьюта примеров, которых локальный гейт не видит, и шаг «ничего не уехало»
(`git diff --exit-code`), ловящий лок примера, если закреплённый тулчейн решает
его иначе, чем закоммиченный. Порядок шагов не косметический: `pub publish
--dry-run` идёт первым, до всего, что пишет в дерево, потому что незакоммиченную
правку он считает предупреждением, а одно предупреждение — это выход 65
(проверено запуском). Заодно закрыт пункт, который находка не назвала, а шаг 0
волны 14 оставил открытым: leak-трекер включён и в сьютах обоих примеров
(`example/*/test/flutter_test_config.dart`), утечек там нет, и проверка
нагруженная — брошенный `ValueNotifier` в выбрасываемом тесте краснит оба сьюта.
...
**Вердикт: не подтвердилось; порядок оставлен, направление находки проверено и
отклонено.** Волна 15, блок порядка освобождения. Оговорка находки оказалась
верной, и вот почему вторичный отказ не воспроизводится: **бросок из
`didUpdateWidget` до состояния больше не доходит.** Элемент выше ловит отказ
обновления (`ComponentElement.performRebuild` оборачивает `updateChild` в
`try/catch`), ставит на место поддерева error-widget и **бросает прежний элемент,
не деактивируя его** — отсюда и утечка, которую видит leak-трекер. Того состояния
никто больше не спрашивает: ни второго `didUpdateWidget`, ни `dispose`. Значит
отменённая подписка в поле так и остаётся никем не тронутой, а следующая
конфигурация строит свежее состояние — что и проверено тестом
`'a selector that raised once does not outlive itself'`.

**Направление находки при этом делает хуже, и это измерено.** Подписка во
временную переменную с отменой старой после неё означает, что упавший селектор
оставляет **старого слушателя на listenable**, который виджет больше не смотрит,
— указывающего в элемент, только что брошенный фреймворком. Проба: после отказа
`hasListeners` у модели `true` при перевёрнутом порядке и `false` при нынешнем.
Вариант с `try/finally` (подписаться, отменить старую в `finally`) даёт ровно то
же конечное состояние, что и нынешний код, то есть меняет только успешный путь.
Поэтому порядок оставлен как есть, а причина записана комментарием на месте — она
неочевидна, и находка родилась именно из этого.
...
**Вердикт: исправлено, но не тем способом, который предлагала находка.** Волна
15, блок решений. Симметрия «сделать как в `runOutsideFrame`» здесь неверна:
`_currentBuildPass()` зовут **из сборки**, то есть чаще всего изнутри кадра, а
`scheduleFrame()` внутри кадра заказывает **следующий** — то есть после каждого
кадра, что-нибудь построившего, появлялся бы ещё один, пустой. У
`runOutsideFrame` этой цены нет: то, что он отложил, пачкает элемент и кадр ему
всё равно нужен, а этот колбэк только сбрасывает поле.

Поэтому кадр заказывается **только когда его некому заказать** —
`schedulerPhase == SchedulerPhase.idle`, то есть та самая сборка вне кадра
(`runApp`), из-за которой находка и написана. Причина и цена записаны
комментарием на месте. Тестом не закрывается, и находка права в том, почему:
убедительного сценария нет — в `runApp` за `attachRootWidget` идёт warm-up-кадр,
так что колбэк отработал бы и без этой правки.
...
**Вердикт: исправлено, и находка привела к соседнему дефекту.** Волна 15, блок
навигации. Цепочка стала `future.whenComplete(…).then(…).onError<Object>(…)`
внутри `unawaited`: обработчик теперь стоит **на цепочке**, а не рядом с
`onValue`, поэтому отвечает и за само действие. Тест —
`'a guard that raises while the node stands aside is reported'`; мутация,
убирающая звено `onError`, роняет его.

Что нашлось по ходу и в находке не названо: **бросок из того же `onValue`
оставлял узел «отступившим в сторону» навсегда.** `_popOutside` поднимал
`_forwarding`, читал `route.popDisposition` и опускал флаг следующей строкой —
а прочитать маршрут — значит прочитать каждый guard на нём, то есть
пользовательский код. Упавший guard оставлял флаг поднятым, и дальше узел отвечал «это не моё
дело» на любое нажатие: маршрут уходил целиком, `onPop` больше не спрашивали.
Лечится `try/finally`; тест — `'a guard that raises does not leave the node
stood aside'`, он падал на `calls == 1` до правки.

Про тесты этой находки стоит знать две вещи, обе выяснены запуском:

- **точку отказа пришлось выбирать.** Первая проба ронялась там, куда указывает
  сама находка, — в пользовательском колбэке, который маршрут делает, уходя
  (`PopScope.onPopInvokedWithResult`). Тест на этом написать нельзя: бросок
  происходит внутри `NavigatorState._flushHistoryUpdates`, и `_debugLocked`
  остаётся поднятым — при разборе дерева прилетает вторичный ассерт
  `'!_debugLocked'` из `NavigatorState.dispose`, и тест краснеет не тем, чем
  надо. Отказ перенесён на одну строку раньше, в чтение `popDisposition`: то же
  `onValue`, тот же пользовательский код, но фреймворк остаётся целым;
- **guard должен быть одноразовым.** Маршрут спрашивают не только из цепочки
  узла: фреймворк читает `popDisposition` на каждом изменении истории
  навигатора, и guard, который ронялся всегда, репортил из мест, к находке не
  относящихся. Он взводится из ответа `onPop` (между взводом и чтением узла —
  только микротаск) и разряжается самим чтением.

Соседей искали в двух направлениях. `Future.then(onValue, onError:)` во всём
`lib/` был ровно один — этот; четыре других `onError:` — это
`Stream.listen(onError:)`, где ошибки `onData` уходят в зону так же, но `onData`
там либо пуст, либо это `_log.d(path)`, то есть пользовательский логгер
(записано в handoff как смежное, не чинилось здесь). Флаг «поднял — сделал —
опустил» без `finally` тоже оказался один: счётчик пересборок
(`beginScopeRebuild`) уже стоит в `try/finally`, а `_buildPassEnding` опускается
пост-кадровым колбэком — это предмет P3-4, а не этой находки.
...
**Вердикт: исправлено первым из двух предложенных путей.** Волна 15, блок
навигации. Конструктор стал приватным (`NodeNavigatorState._()`), тип остался
публичным — им пишут `GlobalKey<NodeNavigatorState>()` и в него же ключ
разрешается. Ассерт не понадобился: класс и раньше был `final`, то есть
унаследовать его снаружи было нельзя, и не хватало ровно запрета на
конструирование. Теперь поставить такое состояние под чужой `Navigator` нельзя
вообще, а не «упадёт на первом попе». Тестом это не покрывается — гарантия
компилятора, как у запечатанных волной 11 хуков; строка в `CHANGELOG` помечена
ломающей.
...
**Вердикт: исправлено, с уточнением к находке.** Волна 15, блок навигации.
`pages` стал полем `_pages`, пересобираемым только в `didUpdateWidget` и только
когда сменились `child` или `isRoot`; `onDidRemovePage` из замыкания в `build`
стал статическим методом. Уточнение: **наблюдателей узел не передаёт вовсе** —
про `observers` находка описывала код до блока навигации волны 14, который убрал
`_HookEntry` и всё, что вокруг него было; в нынешнем `build` есть только `pages`
и `onDidRemovePage`.

Цена, которую находка называет верно, измерена: перестройка узла, ничего не
меняющая, давала **два лишних `NavigationNotification`** — навигатор диффил
стек, флашил историю и рассказывал о ней каждому слушателю выше узла (счётчик в
тесте: было 1, становилось 3). Оба теста — `'a rebuild of the node hands its
navigator the same pages'` (тождество списка и счётчик уведомлений) и `'a changed
child hands its navigator a new page'` (чтобы «держим список» не превратилось в
«держим устаревшую страницу»); мутация `pages: _buildPages()` роняет оба
утверждения первого.

Честная граница правки: помогает она там, где `child` — тот же объект (`const`
поддерево или виджет, который приложение держит в поле). Новый `child` на каждой
перестройке по-прежнему означает новую страницу — иначе она была бы устаревшей.
...
**Вердикт: исправлено; последствие недостижимо, но рядом нашёлся настоящий
дефект.** Волна 15, блок порядка освобождения. Порядок в `dispose` перевёрнут —
сначала `_image?.dispose()`, потом отчёт, — и находка права в том, что это ничего
не стоит. Но описанной потери памяти не бывает: **условия исключают друг друга.**
Успешный захват присваивает `_image` и зовёт `_reportCompleted()` в том же
синхронном шаге (`screenshot_replacer.dart:156-161`), так что «отчитывается
`dispose`» и «есть картинка» одновременно не случаются: когда отчитывается
`dispose`, `_image` равен `null`. Это закреплено тестом
`'when disposal is the one to report, there is no image to release'`.

Зато проба к находке вскрыла дефект, которого в ревью нет: **отказ
`onCompleted` уходил необработанной ошибкой зоны.** Колбэк — пользовательский, и
зовут его оттуда, где никто не ждёт: захват идёт `unawaited`-future в
пост-кадровом колбэке. Бросок оттуда вылезал «в тесте упало» вместо доклада, а на
пути `dispose` — выходил из `State.dispose` и рвал разбор дерева. Теперь
`_reportCompleted` держит `try/catch` и докладывает через
`FlutterError.reportError`, оставаясь одноразовым. Тест — `'a raising onCompleted
is reported, not left in the frame that captured'`, он падал до правки. Соседей
искали по всем пользовательским колбэкам пакета: остальные (`onMount`,
`onUnmount`, `disposeAsync`, `disposeData`, `builder`, `createState`) зовутся из
жизненного цикла скоупа или из `build`, где отказ уже обработан — броском первого
и докладом остальных (волна 14) или границей ошибок сборки.
...
**Вердикт: исправлено, и находка недооценила размах.** Волна 15, блок
диагностики. Тип стал `String`, в дартдоке сказано, что сообщение всегда есть и
что `null` читается как `'null'`. Уточнение: мёртвую ветку получил не
гипотетический автор форматтера, а **сам пакет** — после смены типа анализатор
показал два таких места в собственных тестах: `log.message ?? ''` в
`scope_logger_test.dart` и `'${entry.message}'` в `run_stream_guarded_test.dart`.
То есть лживый тип уже ввёл в заблуждение единственных читателей, которые у него
были. Новый тест — `'a log written with no message reads as null'`.
...
**Вердикт: исправлено, швом, а не приведением типа.** Волна 15, блок
диагностики. У миксина появился `reportName` — по умолчанию `toStringShort()`, а
у элементов обоих семейств, которые его подмешивают, `widget.toStringShort(
showHashCode: true)`. Через тип это сделать нельзя: миксин стоит `on
Diagnosticable` и до `widget` не дотягивается, а научить элемент называть себя
именем виджета в `toStringShort()` — значит читать `widget` из размонтированного
элемента, где он бросает (та же грабля, что и в дефекте контроллера волны 14).
Шов заодно даёт пользовательскому родителю способ назвать себя. Тест — `'a parent
names an expired wait after its widget, tag and all'`; мутация (снять
переопределение) даёт `_TestScopeElement(#21756) couldn't wait…`, то есть ровно
то, на что жалуется находка.
...
**Вердикт: исправлено.** Волна 15, блок диагностики. Строка логирует
`_acquiredScopeKey`, рядом записано почему. Вторая половина находки — про
пользовательский код в замыкании — верна и оказалась тем, чем тест ловится:
`scopeKey` переопределяем, а ленивое сообщение резолвится когда угодно, в том
числе после начала разбора. Тест `'cancelling a pending wait logs the key without
asking again'` считает чтения геттера у конкретного элемента: с живым геттером
их два, с полем — одно. Тест нагружен и обеими половинами: он же проверяет, что
строка в логе есть и называет взятый ключ.
...
**Вердикт: исправлено.** Волна 15, блок диагностики. В `switch` добавлено третье
поле — `hasEntry` — и две ветки для состояния «ключ прочитан, очередь не
занята»: они говорят, что поиск координатора не нашёл никого и что это и есть
уже сообщённый отказ, вместо того чтобы называть несуществующую очередь.
Достижимость подтверждена запуском: `_acquiredScopeKey` присваивается **до**
`AsyncScopeCoordinator._elementOf`, который и бросает, когда координатора нет.
Тест — `'a key changed after a failed lookup is not reported as held'`; мутация,
сворачивающая ветки обратно, даёт «changed while the scope was holding one».
Попутно выяснилось, что ветка «сменился координатор» в этом состоянии
недостижима: `currentCoordinator` вычисляется как `null`, когда
`_acquiredCoordinator` пуст, и охрана наверху выходит раньше.
...
**Вердикт: исправлено ассертом, а не охраной.** Волна 15, блок диагностики.
Молчаливое `if (!isCompleted)` спрятало бы день, когда инвариант перестанет
держаться, — а он держится не на «разборе трёх условий», как предполагала
находка, а на одном: до появления подписки завершить этот completer может только
`_performAsyncDispose`, и он делает это исключительно для инициализации, которая
не начиналась (`_didStartAsyncInit`). Это и записано ассертом с текстом и
комментарием на месте. Тестом не покрывается: состояние недостижимо, падающего
теста на него не напишешь — то же решение, что у охраны `!_isDisposing` в таймере
паузы (P2-13).
...
**Вердикт: исправлено, и наполовину было исправлено раньше.** Волна 15, блок
дартдока. «Defaults to `ScopeConfig.…`» у всех четырёх таймаутов появилось
блоком типов волны 14 (P1-6) — на момент ревью этого не было. Остальное сделано
здесь: у `scopeKey` появилось «Needs an `AsyncScopeCoordinator` above it», у
`waitingBuilder` — «Falls back to [initBuilder] when omitted», у `init`,
`initBuilder` и `errorBuilder` — то же, что у близнеца, у четырёх колбэков
истечения — «reported through `FlutterError.reportError` either way», у
`pauseAfterInitialization` — про `ScopeConfig.pauseAfterInitializationEnabled`.

**Сосед у находки нашёлся сразу:** те же пробелы (кроме `scopeKey`) были у
`AsyncControllerScopeBase`, а одна фраза — про истечение ожидания детей — не была
написана ни у одного из трёх, включая сам `AsyncScope`. Выровнены все три.
...
**Вердикт: исправлено.** Волна 15, блок дартдока. Три статических метода
добавлены с теми же телами, что у соседних слоёв, и с абзацем про то, почему они
там дублируются (статика не наследуется). Тестом не покрывается: это те же
`ScopeContext.maybeOf/of/select`, уже покрытые для двух других слоёв, — новое
здесь только имя, по которому их находят.
...
**Вердикт: исправлена неверная сторона — комментарий кода; темы верны.** Волна
15, блок решений. Тест на описанное окно написан
(`scope_removed_while_initializing_test.dart`, `'says handed over while the ready
branch is still held back'`): скоуп ловится в самом узком состоянии, где флаг и
`_initSucceeded` могли бы разойтись — инициализация зарегистрирована, а ветка
готовности ещё придержана `pauseAfterInitialization`, — и они не расходятся:
ресурс отдаётся ровно один раз, и отдаёт его скоуп.

Почему разойтись они и не могут: **строка под `yield` выполняется, когда поток
просит следующее событие, то есть уже после того, как скоуп забрал выданное.**
Отмена на `yield` заканчивает тело, не доходя до этой строки. А войти в
промежуток между ними снаружи нельзя: возобновление генератора — микротаск, а
дерево меняется в кадре, и кадр всегда позже любого отложенного микротаска.

Комментарий контроллерного семейства переписан: вывод в нём был верный
(критерий должен быть тем же, которым пользуется разбор, — не «двумя фактами,
которые обязаны совпадать»), а обоснование — нет; «a flag would lie» убрано, и
на его место поставлена ссылка на тест. В тему `AsyncScope` добавлен абзац о том,
почему приёму можно доверять; зеркало обновлено.
...
**Вердикт: исправлено в обоих местах.** Волна 15, блок дартдока. И комментарий
`runDispose`, и комментарий теста теперь говорят то, что верно: упавший лист
сохраняет свои ошибки не потому, что его не утилизируют, а потому, что состояние
с ошибками не перезаписывают; утилизируют же его по тому, взял ли инициализатор
что-нибудь, а не по тому, чем он кончил.
...
**Вердикт: исправлено, как и предлагала находка — правкой обещания.** Волна 15,
блок дартдока. Дартдок теперь говорит, что это число шагов, о которых поддерево
отчитается: лист — один, группа считает то, что под ней, и не себя, потому что
своего шага не выдаёт. Заодно сказано, почему это и есть знаменатель для
...
**Вердикт: исправлено, и проверено запуском, а не чтением.** Волна 15, блок
дартдока. Новый тест `'a group keeps one failed child, not every sibling that
fell over'` строит `concurrent`-группу из двух детей, падающих в один и тот же
момент фейковых часов, и показывает: в состоянии ровно одна ошибка, а
`stateToString()` называет одного ребёнка. Тема исправлена в обоих языках.
`join(', ')` оставлен намеренно, с комментарием на месте: диагностика не должна
быть тем, что бросает, а `single` бросил бы — и на пустом списке, и на втором
имени, которого там быть не должно.
...
**Вердикт: не подтвердилось; решения владельца не потребовалось.** Волна 15,
блок решений. Состояние **достижимо публичным путём**, и это проверено запуском:
`ScopeDependency.runDispose()` — публичный метод публичного интерфейса, а
`ScopeAutoDependencies.root` отдаёт корень наружу, так что вызывающий, который
сам подписался на разбор и отменил подписку раньше конца, получает ровно это
состояние. Тест — `'a disposal cancelled by whoever drove it says so'`
(`scope_auto_dependencies_test.dart`): `disposal cancelled`, `isDisposed`
остаётся `false`.

Находка права в другом, и это записано: **пакет сам такого состояния не
производит** — свой разбор скоуп доводит до конца. Условие теперь названо и в
дартдоке класса, и в теме `Scope` (с зеркалом): это состояние про разбор, который
вы ведёте сами. Убирать класс, стало быть, не за что.
...
**Вердикт: исправлено частично — инфраструктурная половина.** Волна 15, блок
инфраструктуры, `b4aa330` и `c625776`. Сделано:

- `.pubignore` исключил `example/*/macos/` (88 файлов, ~690 КБ),
  `example/scopo_demo/web/`, `.vscode/` и новый `/.github/`; список файлов в
  `pub publish --dry-run` стал 323 строки вместо 459. **Уточнение к находке:
  `.vscode/` в архив и не попадал** — он исключён глобально, а не через
  `.gitignore`, и pub спрашивает git. Записан всё равно: на другой машине
  глобального исключения может не быть;
- мёртвых записей в `analysis_options.yaml` оказалось **три, а не две, и
  координаты у находки сдвинуты**: `:43` — это `empty_catches`, живой (правило
  включено набором `core`). Мёртвые — `no_logic_in_create_state: warning`
  (правило выключено ниже, `:84`), `prefer_relative_imports: warning` и
  `unsafe_html: warning` (не включены ни набором, ни локально). Все три убраны.
  То, что severity правило не включает, доказано самим репозиторием: фабрики
  `createState` нарушают первое правило, а `analyze` молчит;
- `example/minimal` и `example/navigation_node` включили строгий корневой конфиг
  и выключили ровно те три правила, что выключает `example/scopo_demo`. Всплыло
  64 замечания, 63 исправлены, одно правило выключено с причиной на месте
  (`no_adjacent_strings_in_list`: уроки — проза, каждый абзац одна строка,
  перенесённая по строчкам внутри списка, и правило не отличает это от забытой
  запятой).

**Вторая половина закрыта блоком дартдока волны 15.** Сделано:

- «9 демо» стало десятью — их и правда десять (`_tabs` в
  `example/scopo_demo/lib/home/home.dart`), в `example/README.md` и в зеркале;
- `README.md` получил третий пример, `navigation_node`, шестью уроками;
- `AsyncScopeModel` больше не «третий типовой аргумент `AsyncScopeCore`»: он
  третий аргумент `ScopeModelCore`, а у `AsyncScopeCore` их два. Исправлено и в
  дартдоке, и в продублировавшей ошибку строке `CHANGELOG`;
- `_NullWidget` бросает `UnimplementedError` с текстом: какая это подстановка,
  чем скоуп строит своё поддерево и что делать. Два теста в `base_test.dart` —
  на сообщение и контрольный на переданный `child`;
- пол Flutter в `CHANGELOG` описан дважды, потому что запись 0.10.0 писалась
  дважды за релиз; ранняя запись теперь прямо говорит, что её отменила поздняя, и
  где смотреть;
- **счётчиков в темах разошлось два, а не четыре.** Проверены все: «Four waits»
  (четыре), «Four things» (четыре), «Two failures» (две), «Two consequences»
  (две), «Three accessors» (три), «Three entry points» (три), «Three methods»
  (три), «Three builders» (три), «Two ways» (две), «Two differences» (две),
  «Two things» (две), «Three cases» (три), «four states» (четыре), «three
  places» (три), «six steps» (шесть). Разошлись `base.md` («Four consequences»
  при пяти абзацах) и `scope_notifier.md` («Four types» при шести типах — у
  четвёртой строки таблицы стояли два имени, а шестого,
  `ScopeStateWithErrorModelView`, не было вовсе). Оба исправлены, вместе с
  зеркалами.

Не сделано и остаётся: `sdk: ^3.6.0` — это отдельная запись бэклога владельца
(тянет tall style и снятие `require_trailing_commas`, 69 файлов).

**Дополнение к вердикту: закрыто целиком, но не той стороной.** 2026-08-18,
`0b39347` и `a70a550`. Владелец отменил подъём `sdk` до `^3.7.0` и опустил пол
Flutter до 3.27.0 — версии, которая несёт ровно Dart 3.6.0. Расхождение, которое
находка и называла, исчезло от этого само: `^3.6.0` стал достижимым, а tall
style, снятие `require_trailing_commas` и реформат 69 файлов сняты с повестки
вместе с записью бэклога. Находка была права в констатации и неявно неправа в
том, что выход у расхождения один: сошлись не подъёмом границы, а спуском пола к
ней. Заплачено тремя ограничениями dev-зависимостей и тремя правками в
`example/scopo_demo`; гейт §6 прогнан на 3.27.0 целиком.
