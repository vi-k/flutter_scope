...
Не сделано и остаётся в этой находке: `sdk: ^3.6.0` — это отдельная запись
бэклога владельца (тянет tall style и снятие `require_trailing_commas`, 69
файлов), и мелочи документации — несовпадающие счётчики в темах, «9 демо» при
десяти вкладках, `README` без третьего примера, `AsyncScopeModel` третьим
типовым аргументом двухпараметрового класса, голый `UnimplementedError` у
`_NullWidget`, пол Flutter дважды в `CHANGELOG`. Они пойдут блоком дартдока и
тем, вместе с P3-14, P3-15, P3-17, P3-18 и P3-19.
