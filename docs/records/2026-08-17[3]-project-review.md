...
Поэтому взят `previous.canPop()`, и в коде рядом написано, что он оставляет:
внешний `PopScope(canPop: false)` по-прежнему обходится. Пометка «решение
принято» требует своей `PopEntry` вместо виджета `PopScope` — то же самое, что
нужно P1-4, чтобы отвечать по живому состоянию, а не по кешу `_innerCanPop`.
Обе части разумно делать одной правкой, и она в этом блоке дальше.
