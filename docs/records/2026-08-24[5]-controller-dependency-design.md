...
> **Состояние на 2026-08-24:** выполнено целиком и смержено в `main`. Одно
> расхождение с дизайном: метод и поле `controller` столкнулись именами в
> `example/scopo_demo` (`FakeController controller` — обычный класс, даже не
> `ScopeController`), обнаружено корневым `flutter analyze`. Решение
> владельца — переименовать поле демки (`fakeController`), не метод. Гейт §6
> пройден целиком, тестов 421 → 423.
> **Что это:** дизайн нового билдера `controller()` в `ScopeAutoDependencies`
> — третьего рядом с `dep()`/`sequential()`/`concurrent()` — оборачивающего
> `ScopeController` в узел дерева зависимостей.
