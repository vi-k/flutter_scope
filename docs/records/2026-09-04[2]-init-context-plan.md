...
> **Состояние на 2026-09-04:** выполнен целиком, работа в `main`
> (`6e92139`…`9325217`). Три отступления от плана: `@nonVirtual` на входе
> движка не поставлен (его потомки обязаны переопределять `initScope`, чтобы
> поймать значение), правка тестов не вынесена в отдельную задачу 7 — без неё
> дерево не собиралось уже в задаче 1, — и примеры переехали не в задаче 8, а
> там, где упирался гейт.
